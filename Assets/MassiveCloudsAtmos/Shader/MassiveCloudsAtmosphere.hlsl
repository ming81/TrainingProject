...
float Shaft(ScreenSpace ss, float currentShaft, float noise)
{
    ...
    float  step      = lerp(0.1, 0.01, _ShaftQuality);

    sunUVDir.y /= _ScreenParams.y / _ScreenParams.x;;
    step = step + 1 * step * ProgressiveShuffler(noise);
    ...
    {
        float  p  = (sunDistUV) * i;
        float2 uv = ss.uv + normalize(sunUVDir) * p;
        const float mergin = 0.0;
        if (uv.x < -mergin || uv.x > 1.0 + mergin || uv.y < -mergin || uv.y > 1.0 + mergin){
          continue;
        }

        uv = saturate(uv);
        float dUV = length(sunUV - uv);
        ...
        float shaftStrength        = _SunShaft * pow(saturate(dotRayDir), 2) * shaftFactor;
        float cloudOcclusionFactor = (oclusionSS.isMaxPlane < 0.95) ? 0 : 1 - pow(cloudFactor.occlusion, 4);

//        shaft += pow(saturate(1 - p), 10) * saturate(cloudOcclusionFactor) * shaftStrength;
        shaft += (1 - shaft) * pow(saturate(1 - p), 10) * saturate(cloudOcclusionFactor) * shaftStrength;
    }

    shaft = saturate(pow(shaft, 0.5));
    float d = saturate(lerp(0.05, 0.5, _ShaftQuality) + abs(shaft - currentShaft));
    return SafeLerp(currentShaft, shaft, 0.5 * pow(d, 2));
    return shaft;
//    return SafeLerp(currentShaft, shaft, 0.1);
}
...
float4 AtmosphereFragment(AtmosphereFactor atmosphereFactor, ScreenSpace ss, float noise, CloudFactor cloudFactor)
{
    ...
    float  cloudDensityFactor    = 1 - _CloudOcclusion * cloudFactor.density;
...
//    atmosphereFactor.scattering = lerp(atmosphereFactor.scattering, pow(scattering, 0.5), 0.5);
...
//    atmosphereFactor.shadow = lerp(atmosphereFactor.shadow, shadow, 0.5);
