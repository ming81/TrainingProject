    ...
    public class Steering : VehicleComponent
    {
        ...
        public override void FixedUpdate()
        {
            ...
            foreach (WheelGroup axle in vc.WheelGroups)
            {
                ...
                {
                    if (vc.wheelbase > 0.001f) // Wheelbase will be larger than 0 only when 4 wheels, 2 per axle.
                    {
                        ...
                        float fiInner = Mathf.Atan(4 * axle.trackWidth * sinAxleAngle /
                                                   (2 * vc.wheelbase * cosAxleAngle - axle.trackWidth * sinAxleAngle));

                        float fiOuter = Mathf.Atan(4 * axle.trackWidth * sinAxleAngle /
                                                   (2 * vc.wheelbase * cosAxleAngle + axle.trackWidth * sinAxleAngle));

                        if (axleAngle < 0)
                        {
                            axle.RightWheel.SteerAngle = fiInner * Mathf.Rad2Deg;
                            axle.LeftWheel.SteerAngle  = fiOuter * Mathf.Rad2Deg;
                        }
                        else
                        {
                            axle.LeftWheel.SteerAngle  = fiOuter * Mathf.Rad2Deg;
                ...
                {
                    foreach (WheelComponent wheel in axle.Wheels) wheel.SteerAngle = axleAngle;
                }
            }
