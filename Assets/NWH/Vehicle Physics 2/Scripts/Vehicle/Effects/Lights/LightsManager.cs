...
namespace NWH.VehiclePhysics2.Effects
{
    ...
    public class LightsMananger : Effect
    {
        ...
        {
            get { return (int) ((Time.realtimeSinceStartup - _leftBlinkerTurnOnTime) * 2) % 2 == 0; }
        }

        public bool RightBlinkerState
        {
            get { return (int) ((Time.realtimeSinceStartup - _rightBlinkerTurnOnTime) * 2) % 2 == 0; }
        }
        ...
        public int GetIntState()
        {
            ...
            SetBit(ref state, brakeLights.On,    0);
            SetBit(ref state, tailLights.On,     1);
            SetBit(ref state, reverseLights.On,  2);
            SetBit(ref state, lowBeamLights.On,  3);
            SetBit(ref state, highBeamLights.On, 4);
            SetBit(ref state, leftBlinkers.On,   5);
            SetBit(ref state, rightBlinkers.On,  6);
            SetBit(ref state, extraLights.On,    7);
